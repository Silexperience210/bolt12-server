...
This safely adds the required protocol flags to `lnd.conf` and restarts LND (~30s).

### Step 2 — Add the community app store
...
Your API key is visible at `http://umbrel.local:3001/api/v1/config` from your local network.
...
- Umbrel (old Docker-based or umbrelOS)
